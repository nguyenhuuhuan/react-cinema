...
export default function CarouselVideoItem({ type, srcVideo, srcPoster, namePorster }: Props) {
  ...
  const handlePlayVideo = (e: OnClick) => {
    ...
    const thumbnailContainer = btnVideoRef.current.querySelector('.thumbnail-container');
    switch (type) {
      case 'video':
      ...
      case 'youtube':
        btnVideoRef.current.insertAdjacentHTML(
          'afterend',
          `<iframe src="${urlYoutube + srcVideo}?autoplay=1"  allowfullscreen></iframe>`
        );
        break;
      default:
        return;
